...
import { checkIsEmail, removeClass } from "../../../ultis/valuedate";
...
const Register = () => {
  ...
  const handleRegister = () => {
    if (name.length === 0) {
      ...
      handleError("Mật khẩu phải từ 8 ký tự trở lên", "password", [
        "name",
        "email",
      ]);
      ...
      handleError(
        "Số điện thoại không hợp lệ. Vui lòng không nhập số 0 đầu tiên",
        "phoneNumber",
        ["name", "email", "password", "password"]
      );
    } else if (phoneNumber.length === 0) {
      handleError("Số điện thoại không được bỏ trống", "phoneNumber", [
        "name",
        "email",
        "password",
        "password",
  ...
  return (
    <Style>
        <div className="form shadow-2xl shadow-white-400 p-10 flex flex-col gap-8">
          ...
          <div className="w-full flex flex-col gap-4 font-serif text-xl">
            ...
            <input
              id="name"
              className="w-full rounded-md p-2 border-solid"
              type="text"
              value={name}
